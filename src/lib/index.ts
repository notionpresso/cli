...
program
  ...
  .option('--fields <fields>', 'List of fields to include in bookmark (comma separated, optional)');
...
const imageOutDir = path.join(process.cwd(), options.imageDir || DEFAULT_IMAGE_OUT_DIR, pageId);

const client = new Client({ auth: options.auth });

const meta = {
  meta: options.meta,
  fields: options.fields ? options.fields.split(',') : options.meta ? DEFAULT_FIELDS : undefined,
};
